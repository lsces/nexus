...
<ul class="{$packageMenuClass}">{else}</li>{/if}
			{if $item.last}</ul>{else}
				<li>
					<div class="{cycle values='even,odd'}">
						...
						<a href="{$smarty.const.NEXUS_PKG_URL}menu_items.php?menu_id={$item.menu_id}&amp;item_id={$item.item_id}">{biticon ipackage="icons" iname="edit" iexplain="edit item" style="float:right" iforce=icon}</a>
		...
		{if $gNexus->mInfo.tree}
			...
			<input type="image" src="{biticon ipackage="icons" iname="user-trash" iexplain='remove item' url=TRUE}" title="remove selected items" style="float:right" />
		{/if}
	{/form}
{/if}
{/strip}
